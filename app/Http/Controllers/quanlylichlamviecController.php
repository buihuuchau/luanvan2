...
class quanlylichlamviecController extends Controller
{
    ...
    public function copylichlamviec(Request $request)
    {
        ...
        if ($sangngay <= $tungay) {
            return back()->withErrors('Không thể thêm lịch cho quá khứ');
        } else {
            $check = DB::table('lichlamviec')
                ->where('idquan', $thanhvien->idquan)
                ->where('thoigian', $sangngay)
                ->get();
            if (count($check) != 0) {
                return back()->withErrors('Ngày ' . $sangngay . ' ' . 'đã có lịch, không thể copy, mời bạn thêm thủ công.');
            } else {
                $lichlamviec = DB::table('lichlamviec')
                    ->where('idquan', $thanhvien->idquan)
